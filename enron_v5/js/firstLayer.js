var FirstLayer = {
	...
	create: function(svg, width, height, data, numberOfClustersAtEachTimeStep) {
		...
		function onRightClickNetworkClusterGroup() {
			...
			// check if the stability and density buttons are selected
			...
			for (var i = 0; i < menuItems.length; i++) {
				menu.push({
					title: menuItems[i],
					currentAttrValue: Database.attribute2DataDict[menuItems[i]][email][currentTimeStep],
					action: function(title, currentAttrValue) {
						TopologicalPointEventHandler.createEventEdit(title, currentAttrValue);
					}
				});
			}
