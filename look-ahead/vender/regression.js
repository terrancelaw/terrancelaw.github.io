const DEFAULT_OPTIONS = { order: 2, precision: 5, period: null };
...
*/
const Regression = {
  linear(data, options = DEFAULT_OPTIONS) {
    const sum = [0, 0, 0, 0, 0];
    let len = 0;
  ...
  exponential(data, options = DEFAULT_OPTIONS) {
  ...
  logarithmic(data, options = DEFAULT_OPTIONS) {
    const sum = [0, 0, 0, 0];
  ...
  power(data, options = DEFAULT_OPTIONS) {
    const sum = [0, 0, 0, 0, 0];
    const len = data.length;
  ...
  polynomial(data, options = DEFAULT_OPTIONS) {
    const lhs = [];
    const rhs = [];
    let a = 0;
    let b = 0;
