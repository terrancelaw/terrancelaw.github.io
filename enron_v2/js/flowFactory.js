var FlowFactory = {
	...
	createFlow: function(name) {
		...
		BarCharts.create(flow, flowWidth, flowHeight, Database.typeDict[name]); // (max width of stream is half of flowWidth)
		FirstLayer.create(flow, flowWidth, flowHeight, Database.sizeDict[name], numberOfClustersAtEachTimeStep);
		SecondLayer.create(flow, flowWidth, flowHeight, Database.sizeDict[name], clusterArray, name.substring(0, name.indexOf("@")));
