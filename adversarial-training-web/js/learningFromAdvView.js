const LearningFromAdvView = {
	...
	init: function() {
		...
		let chartWidth = $('#learning-from-adversarial > .vis-container > .chart > svg').width();
		let chartHeight = $('#learning-from-adversarial > .vis-container > .chart > svg').height();
    	...
    	let svgGroup = d3.select('#learning-from-adversarial > .vis-container > .chart > svg').append('g')
	...
	getTrainErr: function() {
		let trainErr = [];
		let data = Database.transferFromAdversarial;

		trainErr.push(data.attacked[49].train_err);
	...
	getTestErr: function() {
		...
		testErr.push(data.attacked[49].test_err);
	...
	getAdvErr: function() {
		...
		advErr.push(data.attacked[49].adv_err);
	...
	adjustText: function() {
		...
		d3.select(tr_e_text[0]).attr('dx', 10).attr('dy', 0);
		d3.select(tr_e_text[1]).attr('dx', 0).attr('dy', -7).text(function(d) { return d.toFixed(4); });
		d3.select(tr_e_text[2]).attr('dx', 0).attr('dy', -7).text(function(d) { return d.toFixed(4); })
		d3.select(tr_e_text[3]).attr('dx', 0).attr('dy', -7).text(function(d) { return d.toFixed(4); })
		d3.select(tr_e_text[4]).attr('dx', 0).attr('dy', -7).text(function(d) { return d.toFixed(4); })
		d3.select(tr_e_text[5]).attr('dx', 0).attr('dy', -7);

		d3.select(te_e_text[0]).attr('dx', 10).attr('dy', 0);
		d3.select(te_e_text[1]).attr('dx', 0).attr('dy', 15);
		d3.select(te_e_text[2]).attr('dx', 0).attr('dy', 15);
		d3.select(te_e_text[3]).attr('dx', 0).attr('dy', 15);
		d3.select(te_e_text[4]).attr('dx', 0).attr('dy', 15);
		d3.select(te_e_text[5]).attr('dx', 0).attr('dy', 15);

		d3.select(adv_e_text[0]).attr('dx', 0).attr('dy', 15);
